...
rayTracer.render_parallel(scene, image, width, height)
t_calc_image = (Time.monotonic - t1).total_milliseconds

puts "Completed in #{t_calc_image} ms"

to_file = "crystal-raytracer_parallel.png"
...
delta = `echo "'$(git diff --no-index --binary #{to_file} examples/image/crystal-raytracer_example.png)'"`
